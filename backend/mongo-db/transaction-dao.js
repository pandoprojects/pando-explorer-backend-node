...
module.exports = class TransactionDAO {
  ...
  getTransactions(pageNumber, limitNumber, diff, callback) {
    ...
    const sortObject = { 'number': diff === null ? -1 : 1 , timestamp:-1};
    // timestamp:-1
  ...
  getTypeZeroList(address, callback) {
    const now = Math.floor(new Date().getTime() / 1000);
    const startTime = now - 24 * 2 * 60 * 60;
    // console.log("statrt time", startTime.toString());
    let queryObject = { type: 0, "data.outputs.address": address, timestamp: { $gte: startTime.toString() } };

    // console.log("query object", k, queryObject)

    this.client.query(this.transactionInfoCollection, queryObject, function (error, record) {
      if (error) {
        console.log('Transation dao getTotalNumberByHour ERR - ', error);
        callback(error);
      } else {
