...
module.exports = class priceDAO {
  ...
  async getPrice(callback) {
    ...
    if (this.redis !== null) {
      let list = [];
      try {
        list = await this.redis.hmget(redis_key, 'PANDO', 'PTX');
      } catch (e) {
    ...
    function query() {
      const queryObject = { '_id': { $in: ['PANDO', 'PTX'] } };
      self.client.query(self.priceInfoCollection, queryObject, function (error, recordList) {
        if (error) {
          ...
          if (self.redis !== null) {
            self.redis.hmset(redis_key, 'PANDO', JSON.stringify(recordList[0]), 'PTX', JSON.stringify(recordList[1]));
          }
