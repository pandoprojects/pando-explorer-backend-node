...
var { checkPNC721, checkPNC20 } = require('../helper/smart-contract');
...
exports.UpdatePNC721Name = async function () {
  let tokenSummaryInfoList = [];
  try {
    tokenSummaryInfoList = await tokenSummaryDao.getRecordsAsync({ type: "PNC-721", name: "" });
  ...
  for (let i = 0; i < tokenSummaryInfoList.length; i++) {
    ...
    let tokenName = ""
    try {
      tokenName = await _getPNC20Name(address, nameAbi);
    } catch (e) {
    ...
    if (tokenName === "") {
      try {
        tokenName = await _getPNC721Name(address, tokenURIAbi);
      } catch (e) {
        Logger.log('Error in fetch PNC-721 token name by tokenURI in updateTokenHistoryBySmartContract: ', e.message);
      }
...
exports.UpdatePNC20Symbol = async function () {
  let tokenSummaryInfoList = [];
  try {
    tokenSummaryInfoList = await tokenSummaryDao.getRecordsAsync({ type: "PNC-20" });
    Logger.log('tokenSummaryInfoList[0]:', tokenSummaryInfoList[0]);
    Logger.log('tokenSummaryInfoList.length:', tokenSummaryInfoList.length);
  } catch (e) {
    Logger.log('Error occurs in get tokenSummary info:', e.message);
  ...
  for (let i = 0; i < tokenSummaryInfoList.length; i++) {
    ...
    try {
      ...
      Logger.log('Error in fetch symbol in UpdatePNC20Symbol: ', e.message);
    }
...
exports.Execute = async function () {
  ...
  for (let i = 0; i < smartContractIds.length; i++) {
    ...
    const isPNC721 = checkPNC721(abi);
    const isPNC20 = checkPNC20(abi);
    if (!isPNC721 && !isPNC20) {
      Logger.log(`Smart contract #${i}/${smartContractIds.length} Add:${address} is not PNC-721 or PNC-20, skip.`);
      continue;
    }
    ...
    const tokenType = isPNC721 ? 'PNC-721' : 'PNC-20';
    Logger.log('Token type:', tokenType);
    let tokenName = ""
    try {
      tokenName = await _getPNC20Name(address, abi);
    } catch (e) {
      Logger.log('Error in fetch token name by name function in updateTokenHistoryBySmartContract: ', e.message);
    }
    Logger.log('tokenName after name function:', tokenName);
    if (tokenName === "" && isPNC721) {
      try {
        tokenName = await _getPNC721Name(address, abi);
      } catch (e) {
        Logger.log('Error in fetch PNC-721 token name by tokenURI in updateTokenHistoryBySmartContract: ', e.message);
      }
      Logger.log('tokenName after tokenURI function:', tokenName);
    }
    if (tokenName === "" && tokenType === "PNC-20") {
...
async function _getPNC20Name(address, abi) {
  ...
  try {
    ...
    Logger.log('Error occurs in getPNC20Name:', e.message);
    return "";
  }
}

async function _getPNC721Name(address, abi) {
  ...
  try {
    ...
    Logger.log('Error occurs in getPNC721Name:', e.message);
    return "";
  }
}
