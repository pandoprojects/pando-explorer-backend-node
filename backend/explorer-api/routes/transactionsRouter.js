...
var transactionRouter = (app, transactionDao, progressDao, txHistoryDao, config) => {
  ...
  router.get("/transactions/range", (req, res) => {
    ...
    progressDao.getProgressAsync(config.blockchain.networkId)
      .then((progressInfo) => {
        ...
        if (!isNaN(pageNumber) && !isNaN(limit) && pageNumber > 0 && pageNumber <= totalPageNumber && limit > 0 && limit < 100) {
  ...
  router.get("/getWalletamount", (req, res) => {
    let totalPageNumber = 0;
    let pageNumber = 1;
    ...
    progressDao.getProgressAsync(config.blockchain.networkId)
      .then((progressInfo) => {
        ...
        totalPageNumber = Math.ceil(totalNumber / limit);
        let searchPage = pageNumber;
        if (!isNaN(pageNumber) && !isNaN(limit) && pageNumber > 0 && pageNumber <= totalPageNumber && limit > 0 && limit < 1000000000000000000) {
          if (pageNumber > totalPageNumber / 2) {
            diff = limit;
            searchPage = totalPageNumber - pageNumber + 1;
          }
      ...
      .then(transactionInfoList => {
        let filterArr = transactionInfoList.filter(val => val.type === 2).map(val => {
          return {
            wallet: val.data.outputs[0].address,
            Amount: val.data.outputs[0].coins.PTXWei
          }
        });
        var data = ({
          type: 'transaction_list',
